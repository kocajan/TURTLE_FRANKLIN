...
from kobuki_msgs.msg import ButtonEvent, BumperEvent, Sound
...
# Fix topics accordion of the camera

try:
    camera = rospy.get_param('/turtlebot_camera')
except KeyError:
    camera = 'd435'

if camera in ['astra', 'r200']:
    topic_rgb_image = '/camera/color/image_raw'
    topic_depth_image = '/camera/depth_registered/hw_registered/image_rect'
    topic_rgb_camera_info = '/camera/color/camera_info'
...
class Turtlebot(object):
    ...
    def timer_start(self, cb_reference):
        ...
        #timer = rospy.Timer(rospy.Duration(0.001), cb_reference)
